...
public class ClassRoomServiceImpl implements IClassRoomService {
  ...
  @Override
  public void deleteQuizFromClassroom(String classRoomId, String quizId) {
    ClassRoom classRoom =
        classRoomRepository
            .findById(classRoomId)
            .orElseThrow(() -> new RuntimeException("Classroom not found"));
    ...
    // Check QuizId exists in the quizSession
    List<QuizSession> quizSessions =
        quizSessionRepository.findByQuizIdAndClassId(quizId, classRoomId);
    if (!quizSessions.isEmpty()) {
      throw new QuizException(ErrorCode.MESSAGE_QUIZ_EXISTED);
    }
    assignedQuizIds.remove(quizId);
    classRoom.setAssignedQuizIds(assignedQuizIds);
    classRoomRepository.save(classRoom);
    // Remove the classRoomId from the quiz's classIds
    Quiz quiz =
        quizRepository
            .findById(quizId)
            .orElseThrow(() -> new QuizException(ErrorCode.MESSAGE_INVALID_ID));
