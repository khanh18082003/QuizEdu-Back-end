...
public class QuizServiceImpl implements IQuizService {
    ...
    public QuizCreationResponse save(QuizCreationRequest request) {
        ...
        if (request.multipleChoiceQuiz() != null) {
            ...
            if (request.multipleChoiceQuiz().getQuestions() != null) {
                request.multipleChoiceQuiz().getQuestions().forEach(q -> {
    ...
    @Override
    public QuizCreationResponse addMultipleChoiceQuizQuestion(String quizId, List<QuestionMultipleChoiceRequest> questions) {
        MultipleChoiceQuiz multipleChoiceQuiz = multipleChoiceQuizRepository.findByQuizId(quizId);
        Quiz quiz = quizRepository.findById(quizId).orElseThrow(() -> new RuntimeException("Quiz not found"));
        List<MultipleChoiceQuiz.Question> newQuestions = questions.stream()
                .map(req -> {
    ...
    @Override
    public QuizCreationResponse updateMultipleChoiceQuizQuestion(String quizId, List<UpdateQuestionMultipleChoiceRequest> questions) {
        Quiz quiz = quizRepository.findById(quizId)
                .orElseThrow(() -> new RuntimeException("Quiz not found"));
        ...
        questions.forEach(req -> {
            MultipleChoiceQuiz.Question question = originalQuestions.stream()
                    .filter(q -> q.getQuestionId().equals(req.questionId()))
                    .findFirst()
                    .orElseThrow(() -> new RuntimeException("Question not found: " + req.questionId()));
