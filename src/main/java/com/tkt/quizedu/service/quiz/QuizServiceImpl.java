...
public class QuizServiceImpl implements IQuizService {
  ...
  public List<MatchingQuiz.UserAnswer> getMatchingHistoryByUserId(
    ...
    @Override
    @Transactional
    public QuizResponse updateMatchingQuizQuestion(String quizId, UpdateMatchingQuestionRequest request) {
        ...
        for (UpdateMatchingQuestion req : request.getQuestions()) {
            ...
            MatchingQuiz.MatchPair pair = originalPairs.stream()
                    .filter(q -> q.getId().equals(req.getId()))
                    .findFirst()
                    .orElseThrow(() -> new QuizException(ErrorCode.MESSAGE_INVALID_ID));

            // ===== Xử lý itemA =====
            MatchingType typeA = req.getTypeA() != null ? req.getTypeA() : pair.getItemA().getMatchingType();
            String contentA;
            ...
            // ===== Xử lý itemB =====
            MatchingType typeB = req.getTypeB() != null ? req.getTypeB() : pair.getItemB().getMatchingType();
            String contentB;
